// adapted from https://raw.githubusercontent.com/rust-lang/rust/refs/heads/master/library/std/src/sys/pal/unix/os.rs
...
use std::{fmt, io, iter, mem, ptr, slice, str, vec};
...
fn cvt(val: c_int) -> io::Result<c_int> {
    if val != -1 {
        Ok(val)
    } else {
...
fn confstr(key: c_int, size_hint: Option<usize>) -> io::Result<OsString> {
    ...
    unsafe {
        ...
        assert_eq!(
            last_byte,
            Some(0),
            "`confstr` provided a string which wasn't nul-terminated"
        );
    };
...
fn darwin_temp_dir() -> PathBuf {
    confstr(libc::_CS_DARWIN_USER_TEMP_DIR, Some(64))
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
