...
macro_rules! dlsym {
    ...
    ($v:vis fn $name:ident($($t:ty),*) -> $ret:ty, $sym:expr) => (
        ...
        $v static $name: $crate::unix_futex::unix_weak::DlsymWeak<unsafe extern "C" fn($($t),*) -> $ret> =
            $crate::unix_futex::unix_weak::DlsymWeak::new(concat!($sym, '\0'));
    );
}
pub(crate) use dlsym;
...
impl<F> DlsymWeak<F> {
    ...
    unsafe fn initialize(&self) -> Option<F> {
        ...
        if val.is_null() {
            ...
            Some(mem::transmute_copy::<*mut libc::c_void, F>(&val))
        }
    }
}
